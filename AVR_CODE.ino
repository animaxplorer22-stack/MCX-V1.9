...
// ==================== VERSION ====================
#define VERSION "9.1-FIXED"
#define MINER_TYPE "avr"

// ==================== USER CONFIGURATION ====================
const char USERNAME[] PROGMEM = "XAVER123";
const char PRIVATE_KEY[] PROGMEM = "MCR_A87D9AF718F62C8D073FDDFE6BC0F039";

// ==================== BAUD RATE ====================
#define SERIAL_BAUD 115200  // Working!
...
void validate_miner_state() {
  ...
  if (state.stake < LEVEL_STAKE_RANGE) { state.stake = LEVEL_STAKE_RANGE; needsSave = 1; }
  if (state.stake > 1000000) { state.stake = 1000000; needsSave = 1; }
...
void force_reset_eeprom() {
  Serial.println("{\"type\":\"eeprom\",\"status\":\"resetting\"}");
  ...
  Serial.println("{\"type\":\"eeprom\",\"status\":\"reset_complete\",\"level\":1,\"stake\":1000}");
}
...
uint8_t loadEEPROM() {
  ...
  // If corrupt, force reset
  if (magic != MAGIC_NUMBER || chk != calcChecksum() || version != EEPROM_VERSION) {
    Serial.println("{\"type\":\"eeprom\",\"status\":\"corrupt\",\"action\":\"resetting\"}");
    force_reset_eeprom();
    return 0;
  }
  ...
  // If data was corrupted and fixed, save
  saveEEPROM();
...
void setup() {
  ...
  // ========== LOAD EEPROM (with auto-reset if corrupt) ==========
  loadEEPROM();
  calcLevel();
  
  // Diagnostic output
  Serial.println("{\"type\":\"diagnostic\",\"level\":1,\"stake\":1000,\"status\":\"ready\"}");
