...
def djb2_hash(data: str) -> str:
    """DJB2 hash - 8 character hex output - DETERMINISTIC"""
    h = 5381
    for c in data:
        h = ((h << 5) + h) + ord(c)
...
def verify_signature(pub: str, msg: str, sig: str, miner_type: str) -> bool:
    ...
    """
    # ✅ FIX: Added 'avr' and 'uno' support with debug logging
    if miner_type in ["uno", "avr", "web"]:
        expected = djb2_hash(f"{pub}{msg}")
        logger.debug(f"[SIG] AVR/Web: pub={pub[:20]}..., msg={msg[:30]}..., expected={expected}, got={sig}")
        
        # Case-insensitive comparison for hex
        result = sig.lower() == expected.lower()
        if not result:
            logger.warning(f"[SIG] ❌ Signature mismatch! Expected: {expected}, Got: {sig}")
        else:
            logger.debug(f"[SIG] ✅ Signature verified for {miner_type}")
        return result
...
class DEX:
    ...
    async def create_genesis_tokens(self):
        ...
        if result["success"]:
            logger.info(f"[GENESIS] Created MCX/USDC pool")
        ...
        if result["success"]:
            logger.info(f"[GENESIS] Created MCX/ETH pool")
        ...
        if result["success"]:
            logger.info(f"[GENESIS] Created MCX/BTC pool")
        
        # Initial bridge balances
        bridge.tokens["USDC"]["balance"] = 100000
        bridge.tokens["ETH"]["balance"] = 100
        bridge.tokens["BTC"]["balance"] = 10
        bridge._update_bridge_balance("USDC", 100000)
        bridge._update_bridge_balance("ETH", 100)
        bridge._update_bridge_balance("BTC", 10)
        logger.info(f"[GENESIS] Bridge funded with REAL tokens")
...
class MicroCoreNetwork:
    def __init__(self, is_genesis: bool, username: str, wallet: str, priv: str, pub: str):
        ...
        # Initialize DEX and Real Token Bridge
        self.dex = DEX(self)
    ...
    def register_miner(self, vid: str, pub: str, username: str, wallet: str, stake: int, sig: str, timestamp: int, mtype: str = "web") -> bool:
        """
        Register a miner with enhanced debugging for AVR/Web miners
        """
        # ✅ FIX: Detailed debug logging
        logger.info(f"[REGISTER] ───────────────────────────────────────")
        logger.info(f"[REGISTER] vid={vid[:16]}...")
        ...
        logger.info(f"[REGISTER] msg={msg[:30]}...")
        
        # ✅ FIX: Verify signature with explicit type handling
        if not verify_signature(pub, msg, sig, mtype):
            logger.warning(f"[REGISTER] ❌ Invalid signature from {username} (type: {mtype})")
            logger.warning(f"[REGISTER] pub={pub[:20]}...")
            logger.warning(f"[REGISTER] msg={msg[:30]}...")
            logger.warning(f"[REGISTER] sig={sig[:16]}...")
            logger.warning(f"[REGISTER] Expected DJB2: {djb2_hash(f'{pub}{msg}')[:16]}...")
            return False
        
        logger.info(f"[REGISTER] ✅ Signature verified for {username} (type: {mtype})")
    ...
    async def _handle_ws_message(self, websocket, data: dict, msg_type: str, client_ip: str):
        try:
            ...
            if msg_type == "register":
                ...
                # ✅ FIX: Log all registration data for debugging
                logger.debug(f"[WS] Registration data: validator_id={data.get('validator_id', '')[:16]}...")
                logger.debug(f"[WS] public_key={data.get('public_key', '')[:20]}...")
                logger.debug(f"[WS] wallet={data.get('wallet', '')[:16]}...")
                logger.debug(f"[WS] stake={data.get('stake', 0)}")
                logger.debug(f"[WS] timestamp={data.get('timestamp', 0)}")
                logger.debug(f"[WS] signature={data.get('signature', '')[:16]}...")
